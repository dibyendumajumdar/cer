...
import org.redukti.cer.utils.NumberUtils;
 ...
 *
 * @see org.redukti.cer.parser.Parser
 * @author Mike McCabe
 * @author Brendan Eich
 */
public class TokenStream {
    ...
    private static int stringToKeyword(String name, int version, boolean isStrict) {
        if (version < Versions.VERSION_ES6) {
    ...
    final int getToken() throws IOException {
        ...
        for (; ; ) {
                ...
                if (!containsEscape) {
                    ...
                    if (result != Token.EOF) {
                        if ((result == Token.LET || result == Token.YIELD)
                                && parser.compilerEnv.getLanguageVersion() < Versions.VERSION_1_7) {
                        ...
                        if (result != Token.RESERVED) {
                            return result;
                        } else if (parser.compilerEnv.getLanguageVersion() >= Versions.VERSION_ES6) {
                            return result;
            ...
            if (isDigit(c) || (c == '.' && isDigit(peekChar()))) {
                ...
                boolean es6 = parser.compilerEnv.getLanguageVersion() >= Versions.VERSION_ES6;
                ...
                if (base == 10 && !isInteger) {
                    ...
                    dval = NumberUtils.stringPrefixToNumber(numString, 0, base);
                }
            ...
            switch (c) {
                ...
                case '*':
                    if (parser.compilerEnv.getLanguageVersion() >= Versions.VERSION_ES6) {
                        if (matchChar('*')) {
                            if (matchChar('=')) {
    ...
    final String getLine() {
        ...
        if (lineEndChar >= 0) {
            ...
            for (; ; ++lineLength) {
                ...
                if (c == EOF_CHAR || ParserRuntime.isJSLineTerminator(c)) {
                    break;
                }
            }
