...
import org.redukti.cer.utils.Messages;
...
public class Parser {
    ...
    private AstNode parseFunctionBody(int type, FunctionNode fnNode) throws IOException {
        ...
        if (!matchToken(Token.LC, true)) {
            if (compilerEnv.getLanguageVersion() < Versions.VERSION_1_8
    ...
    private FunctionNode function(int type, boolean isGenerator) throws IOException {
        ...
        if (matchToken(Token.NAME, true)) {
            ...
            // Anonymous function:  leave name as null
                ...
                && (compilerEnv.getLanguageVersion() >= Versions.VERSION_ES6)) {
    ...
    private Loop forLoop() throws IOException {
        ...
        try {
            ...
            } else if (compilerEnv.getLanguageVersion() >= Versions.VERSION_ES6
            ...
            while (matchToken(Token.CATCH, true)) {
                ...
                switch (peekToken()) {
                    case Token.LP:
                        ...
                        break;
                    case Token.LC:
                        if (compilerEnv.getLanguageVersion() >= Versions.VERSION_ES6) {
        ...
        for (; ; ) {
            ...
            switch (tt) {
                ...
                case Token.SHNE:
                    consumeToken();
                    int parseToken = tt;
                    if (compilerEnv.getLanguageVersion() == Versions.VERSION_1_2) {
    ...
    private ObjectProperty plainProperty(AstNode property, int ptt) throws IOException {
        ...
        if ((tt == Token.COMMA || tt == Token.RC)
                && ptt == Token.NAME
                && compilerEnv.getLanguageVersion() >= Versions.VERSION_1_8) {
            if (!inDestructuringAssignment
                    && compilerEnv.getLanguageVersion() < Versions.VERSION_ES6) {
    ...
    private AstNode createNumericLiteral(int tt, boolean isProperty) {
        ...
        if (this.inUseStrictDirective && ts.isNumericOldOctal()) {
            if (compilerEnv.getLanguageVersion() >= Versions.VERSION_ES6 || !isProperty) {
                if (tt == Token.BIGINT) {
    ...
    protected void checkActivationName(String name, int token) {
        ...
        if ("arguments".equals(name)
            ...
            if (token == Token.GETPROP && compilerEnv.getLanguageVersion() == Versions.VERSION_1_2) {
    ...
    private int lineBeginningFor(int pos) {
        ...
        while (--pos >= 0) {
            char c = buf[pos];
            if (ParserRuntime.isJSLineTerminator(c)) {
